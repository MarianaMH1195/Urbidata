...
def run_download():
    ...
    Si ya existen archivos descargados en data/raw/, los reutiliza
    sin volver a descargar (caché local)
    """
    #COMPROBAR SI ESTÁN LOS DATOS EN RAW (CACHÉ LOCAL) 
    archivos_existentes = [f for f in os.listdir(config.RAW_DIR) if f.endswith(".csv.gz")]
 
    if archivos_existentes:
        print(f"\n✅ Caché encontrada: {len(archivos_existentes)} archivo(s) en {config.RAW_DIR}")
        return
    #DESCARGAR DATOS
    print("Iniciando descarga de datos MITMA...")
